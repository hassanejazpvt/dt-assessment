...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobs(int $userId)
    {
        ...
        if ($user) {
            if ($user->is(UserEnum::TYPE_CUSTOMER)) {
                $jobs = $userService->getUserJobs($userId);
                $userType = UserEnum::TYPE_CUSTOMER;
            } else if ($user->is(UserEnum::TYPE_TRANSLATOR)) {
                $jobs = $jobService->getTranslatorJobs($userId);
                $userType = UserEnum::TYPE_TRANSLATOR;
            }
        }

        foreach ($jobs as $job) {
            if ($job->immediate == JobEnum::YES) {
                $emergencyJobs[] = $job;
            } else {
                $normalJobs[] = $job;
            }
        }
        $normalJobs = $jobService->updateUserCheck($normalJobs, $userId);

        return ['emergency_jobs' => $emergencyJobs, 'normal_jobs' => $normalJobs, 'user' => $user, 'user_type' => $userType];
    }
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAllJobs(BookingDTO $bookingDTO, AuthenticatedUserDTO $authenticatedUserDTO): array
    {
        ...
        return $jobService->getAllJobs($bookingDTO, $authenticatedUserDTO);
    }
